...
class KabumScraper(BaseScraper):
    ...
    async def fetch(self, keyword: str, client: Any) -> str:
        ...
        # and await client.get(...) or page.goto(...)
        # For now, we simulate a successful fetch that would be intercepted in tests.
    ...
    def parse(self, document: str, keyword: str) -> List[PriceContract]:
        """
        Parses Kabum HTML to extract products.
        """
        ...
        for card in cards:
            try:
                ...
                price_cash_elem = card.find(class_="priceCard")
                ...
                if price_cash is None:
                    continue  # A product without a price is invalid or out of stock

                price_inst_elem = card.find(class_="priceCardInstallments")
                price_inst_str = price_inst_elem.text.strip() if price_inst_elem else ""
                price_inst = self._clean_price(price_inst_str)

                # Kabum doesn't explicitly mark out of stock with standard pricing UI,
                # but if there's a price, we'll assume it's available for this mock.
                is_available = True
